...
const blankOffice = () => ({ officeId: crypto.randomUUID(), name: "", ssid: "", bssid: "", privateIp: "", ipPrefix: "", active: true, status: "approved" });
...
export default function OfficeWifiSettings() {
  ...
  return <main className="screen officeWifiPage"><div className="wide">
    ...
    <div className="wifiPolicyNotice"><MapPin size={22}/><div><b>Admin approval controls attendance access</b><span>Employees with the Set Office Wi-Fi feature can submit their current network. New submissions remain pending until you enable them here. Only enabled networks are accepted by attendance.</span></div></div>
    <section className="officeWifiGrid">{offices.map((office, index) => <article className={`panel officeWifiCard ${office.active ? "active" : "inactive"}`} key={office.officeId}>
      <div className="officeWifiHeading"><span><Building2 size={21}/><b>{office.name || `Office ${index + 1}`}</b></span><label className="wifiActive"><input type="checkbox" checked={office.active} onChange={event => change(office.officeId, "active", event.target.checked)}/><span>{office.active ? "Approved & Active" : "Pending Approval"}</span></label></div>
      {office.submittedByEmployeeId && <div className="wifiSubmissionMeta"><b>Submitted by {office.submittedByEmployeeId}</b><span>{office.submittedAt ? new Date(office.submittedAt).toLocaleString() : "Employee submission"}</span></div>}
      <div className="officeWifiFields"><label><span>Office name *</span><input value={office.name} onChange={event => change(office.officeId, "name", event.target.value)} placeholder="Noida Head Office"/></label><label><span>Wi-Fi name (SSID)</span><input value={office.ssid} onChange={event => change(office.officeId, "ssid", event.target.value)} placeholder="Globalbirth_Office"/></label><label><span>Router BSSID</span><input value={office.bssid} onChange={event => change(office.officeId, "bssid", event.target.value)} placeholder="aa:bb:cc:dd:ee:ff"/></label><label><span>Captured private IPv4</span><input value={office.privateIp || ""} onChange={event => change(office.officeId, "privateIp", event.target.value)} placeholder="192.168.1.25"/></label><label><span>Local IP prefix</span><input value={office.ipPrefix} onChange={event => change(office.officeId, "ipPrefix", event.target.value)} placeholder="192.168.1."/></label></div>
