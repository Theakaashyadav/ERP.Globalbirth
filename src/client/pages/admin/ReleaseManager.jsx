...
export default function ReleaseManager() {
  ...
  const [notes,setNotes]=useState(""); const [publishing,setPublishing]=useState(false); const toast=useToast();
  async function load(){try{const [latestResponse,candidateResponse]=await Promise.all([fetch("/api/app-update/latest",{headers:headers()}),fetch("/api/app-update/candidate",{headers:headers()})]);const latestData=await latestResponse.json();const candidateData=await candidateResponse.json();setLatest(latestData.available?latestData.release:null);if(!candidateResponse.ok||!candidateData.success)throw new Error(candidateData.message||"Version detection failed.");setCandidate(candidateData.data)}catch(error){toast.error(error.message)}}
  useEffect(()=>{load()},[]);
  async function publish(event){event.preventDefault();setPublishing(true);try{const response=await fetch("/api/app-update/publish",{method:"POST",headers:{...headers(),"Content-Type":"application/json"},body:JSON.stringify({notes})});const data=await response.json();if(!response.ok||!data.success)throw new Error(data.message||"Alert failed.");toast.success(`Mandatory update ${data.release.versionName} alert sent.`);await load()}catch(error){toast.error(error.message||"Could not send update alert.")}finally{setPublishing(false)}}
  return <main className="screen"><div className="wide"><PageHeader icon={Rocket} title="Android Release Alerts" subtitle="Send mandatory update alerts for the APK already published in GitHub." tone="cyan" />
    <section className="releaseGrid"><section className="panel releaseFormPanel"><div className="releasePanelHeading"><Send size={26}/><div><span className="eyebrow">NEW MANDATORY ALERT</span><h2>Release GitHub APK</h2></div></div><form onSubmit={publish}>
      ...
      <button className="btn full cyan" disabled={publishing||!candidate}><Send size={18}/>{publishing?"Checking GitHub & Sending...":"Send Mandatory Update Alert"}</button>
    </form></section>
